...
def _encode_list_field(x):
    # If it's already a list -> json dump
    if isinstance(x, list):
        return json.dumps(x, ensure_ascii=False)
    # if nan or empty -> json dump empty list
    if pd.isna(x):
        return json.dumps([])
    # if already a JSON-like string starting with [ -> keep
    if isinstance(x, str) and x.strip().startswith("["):
        return x
    # fallback: return json of string in list
...
def load_data():
    ...
    try:
        ...
        # if file corrupted or unreadable, return empty dataframe
...
def save_data(df):
    # convert list columns to JSON strings for storage in Excel
    df2 = df.copy()
    for col in LIST_COLS:
...
# Allow admin password to be stored in Streamlit secrets
ADMIN_PASSWORD = st.secrets.get("ADMIN_PASSWORD", ADMIN_PASSWORD) if st._is_running_with_streamlit else ADMIN_PASSWORD
    ...
    if pwd != ADMIN_PASSWORD:
        ...
        with tab1:
            ...
            with st.form("add_form"):
                ...
                # load current street list from existing data to avoid duplicates
                df_tmp = load_data()
                street_options = sorted([s for s in df_tmp['Đường'].dropna().unique().tolist()])
                # common defaults appended
                default_streets = [
                    "An Hội", "Nguyễn Văn Khối", "Lê Văn Thọ", "Quang Trung", "Tân Sơn"
                ]
                # merge while keeping unique
                combined_streets = sorted(list(dict.fromkeys(default_streets + street_options)))
...
# -----------------------
    ...
    # prepare options from data (fallback to defaults)
    ...
    loc_ngay = st.date_input("Ngày trống trước ngày (tuỳ chọn)", value=None)
    ...
    # list contains checking (loai, noi that, tien ich)
    if loc_loai:
    ...
    # date filter (only if column exists and notna)
    if loc_ngay:
        if DATE_COL in df_filtered.columns:
            df_filtered = df_filtered[df_filtered[DATE_COL].notna()]
            df_filtered = df_filtered[df_filtered[DATE_COL] <= pd.to_datetime(loc_ngay).date()]
    ...
    # show only address and minimal info for privacy (requirement: nhân viên chỉ xem địa chỉ)
    if not df_filtered.empty:
        ...
        # download filtered results (full data)
